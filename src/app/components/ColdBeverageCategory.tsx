...
import { ColdBeverageCardProps } from '../../../Types/types';

const ColdBeverageCategory = ({coldBeverage}: ColdBeverageCardProps) => {
  return (
    <>
    <div className="bg-gradient-to-tl to-[#f4d1b7] from-[#7e5233] border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700 xl:h-[60vh] hover:cursor-pointer hover:scale-105 hover:transition-all duration-300 hover:ease-linear shadow-md shadow-[#674632] overflow-hidden">
        <div className="">
          <Link href={`/products/${coldBeverage.id}`}></Link>
        </div>
        <Image
          src={coldBeverage.image}
          alt={coldBeverage.name}
          height={200}
          width={200}
        ...
        <p className="text-center text-[15px] px-[10px]">{coldBeverage.description}</p>
        <span className="font-bold text-[#804a29] pl-[3rem] mt-6">
          Price: ${coldBeverage.price}
        </span>
...
export default ColdBeverageCategory
